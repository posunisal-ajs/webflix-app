...
function uploadFile(
	...
	// ****************************************
	// INSERIDO PRODWEB - 25/03/2014
	// Verifica path destino e cria, se necessário.
	
	$path = $toDir;

	if (strpos($path, ":\\") === false) $pathToken = "/";
	else $pathToken = "\\";
	
	$path = rtrim($path, "/") . "/";
	
	$pathTmp = explode($pathToken, $path);
	$pathTest = "";
	
	for ($i = 0; $i < count($pathTmp); $i++) {
		$pathTest.= $pathTmp[$i] . $pathToken;
		if (file_exists($pathTest) === false) mkdir($pathTest, 0755);
	}
