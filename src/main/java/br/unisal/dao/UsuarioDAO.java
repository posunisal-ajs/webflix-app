...
public class UsuarioDAO extends GenericDAO implements DAOInterface<Usuario> {
	...
	public void update(Usuario t, Long id, Boolean ctrlSenha) throws ClassNotFoundException, SQLException, IOException {
		String sqlSenha = "";
		if (ctrlSenha)sqlSenha = ", senha = ?";
		System.out.println("Sql senha" + sqlSenha);
		String sql = "update usuario set nome = ?, email = ?, login = ?" + sqlSenha + " where id = ?";
		PreparedStatement ps = null;
		try {
			ps = getConnectionPool().prepareStatement(sql);
	...
	public boolean checkPassword(String senha, Long id) throws ClassNotFoundException, SQLException, IOException 
    {	String sql = "select uuid from usuario where senha=? and id=?";
    	 Usuario usuario = null;
		 PreparedStatement ps = null;
		 ResultSet rs = null;
		 DbUtil.getInstance().closeQuietly(ps, rs);
     	try {
	        ...
	        while (rs.next()) {
				usuario = new Usuario();
				usuario.setUuid(rs.getString(1));
			}
		} catch(Exception erro) {
			System.out.println("Erro ocorrido: n" + erro);
		} finally {
		...
		if (usuario != null) {
			System.out.println("USER EDIT = " + usuario);
			return true;
		} else {
			return false;
		}                 
	 }
	public boolean checkEmail(String email, Long id) throws ClassNotFoundException, SQLException, IOException 
    {	String sql = "select uuid from usuario where email=? and id <> ?";
    	 Usuario usuario = null;
		 PreparedStatement ps = null;
		 ResultSet rs = null;
		 DbUtil.getInstance().closeQuietly(ps, rs);
	...
	public boolean checkEmailNew(String email) throws ClassNotFoundException, SQLException, IOException 
    {	String sql = "select uuid from usuario where email=?";
    	 Usuario usuario = null;
		 PreparedStatement ps = null;
		 ResultSet rs = null;
		 DbUtil.getInstance().closeQuietly(ps, rs);
